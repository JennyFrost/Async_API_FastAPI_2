...
class PersonService(CacheMixin):
    ...
    async def search_person(
        ...
        if not persons:
            ...
            # )
            self.elastic_main.search(query=search_text, query_field='full_name').paginate(page=page, page_size=page_size)
            persons = await self.elastic_main.get_queryset(index='persons', some_class=Person)
            result = []
    ...
    async def get_by_id(self, person_id: str) -> Person | None:
        ...
        if not person:
            person = await self.elastic_main.get_by_id(person_id, "persons", Person)
            if not person:
                return None
            person = await self._get_person_roles_from_elastic(person)
    ...
    async def _get_person_roles_from_elastic(self, person: Person) -> Person | None:
        ...
        #     index='movies')
        self.elastic_main.filter({"actors": person.uuid, "director": person.uuid, "writers": person.uuid}).inner_objects(source_field='id')
        films = await self.elastic_main.get_queryset(index='movies')
