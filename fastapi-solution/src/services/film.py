...
class FilmService(CacheMixin):
    ...
    async def get_by_id(self, film_id: str) -> Film | None:
        ...
        if not film:
            film = await self.elastic_main.get_by_id(film_id, "movies", Film)
            if not film:
                return None
    ...
    async def get_films_query(self, page: int, page_size: int, query: str) -> list[FilmBase]:
        ...
        if not films:
            ...
            # )
            self.elastic_main.search(query=query, query_field='title').paginate(page=page, page_size=page_size)
            films = await self.elastic_main.get_queryset(index='movies', some_class=FilmBase)
            if not films:
                return []
    ...
    async def get_films_page(self, page: int, size: int, sort_field: str, genre: str):
        ...
        if not page_films:
            ...
            #     )
            self.elastic_main.get_all().paginate(page=page, page_size=size).filter(dict_filter=dict_filter).sort(sort_field=sort_field)
            page_films = await self.elastic_main.get_queryset(index='movies', some_class=FilmBase)
            if not page_films:
                return []
    ...
    async def get_person_films(
        ...
        if not person_films:
            ...
            #     sort_field=sort_field)
            self.elastic_main.get_all().paginate(page=page_number, page_size=page_size).filter(dict_filter={"actors": person_id, "director": person_id, "writers": person_id}).sort(
                sort_field=sort_field)
            person_films = await self.elastic_main.get_queryset(index='movies', some_class=FilmBase)
