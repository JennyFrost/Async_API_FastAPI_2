...
class GenreService:
    ...
    async def get_by_id(self, genre_id: str) -> Genre | None:
        ...
        if not genre:
            genre = await self.elastic_conn.get_by_id(obj_id=genre_id, index="genres", some_class=Genre)
            if not genre:
                return None
    ...
    async def get_genres_list(self, page: int, page_size: int) -> list[Genre]:
        ...
        if not genres:
            self.elastic_conn.get_all().paginate(page=page, page_size=page_size)
            genres = await self.elastic_conn.get_queryset(index='genres', some_class=Genre)
            if not genres:
                return []
...
def get_genres_service(
    ...
    return GenreService(elastic_conn, redis_conn)
